...
export class FileSystemAdapter implements DataAdapter, AnnouncementAdapter {
  ...
  async getRuleModifications(): Promise<RuleModification[]> {
    const directory = path.join(process.cwd(), 'cboa-site', this.contentPath)
  ...
  async getRuleModification(slug: string): Promise<RuleModification | null> {
    const filePath = path.join(process.cwd(), 'cboa-site', this.contentPath, `${slug}.md`)
  ...
  async createRuleModification(rule: Omit<RuleModification, 'id' | 'createdAt' | 'updatedAt'>): Promise<RuleModification> {
    ...
    const filePath = path.join(process.cwd(), 'cboa-site', this.contentPath, `${id}.md`)
  ...
  async getAnnouncements(): Promise<Announcement[]> {
    const directory = path.join(process.cwd(), 'cboa-site', 'content/portal/announcements')
    ...
    for (const file of files) {
      if (file.endsWith('.md')) {
        ...
        announcements.push({
          ...
          type: data.type || 'info',
          date: data.date || new Date().toISOString(),
          author: data.author || 'CBOA Executive',
          createdAt: data.createdAt,
          updatedAt: data.updatedAt,
        })
      }
  ...
  async getAnnouncement(id: string): Promise<Announcement | null> {
    const filePath = path.join(process.cwd(), 'cboa-site', 'content/portal/announcements', `${id}.md`)
