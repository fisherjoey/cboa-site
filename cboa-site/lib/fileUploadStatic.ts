...
export async function uploadFileStatic(file: File): Promise<{ url: string; fileName: string; size: number }> {
  ...
  const resources = JSON.parse(localStorage.getItem('cboa_resources') || '[]')
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      // In production, the file would be saved to: cboa-site/public/portal/resources/
      ...
      localStorage.setItem('cboa_resources', JSON.stringify(resources))
...
// 1. Files would be uploaded to: cboa-site/public/portal/resources/
