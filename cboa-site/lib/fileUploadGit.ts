  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      try {
        ...
        const manifests = JSON.parse(localStorage.getItem('cboa_file_manifests') || '[]')
        manifests.push(manifest)
        localStorage.setItem('cboa_file_manifests', JSON.stringify(manifests))
        
        // For actual file data, save temporarily in localStorage
        // In production, the file would be committed to git
        localStorage.setItem(`cboa_file_${fileName}`, base64)
...
export function getFileUrl(fileName: string): string {
  ...
  const fileData = localStorage.getItem(`cboa_file_${fileName}`)
  if (fileData) {
