  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      if (body.action === 'request_invite') {
        ...
        await sendEmailViaMicrosoftGraph(msToken, normalizedEmail, EMAIL_SUBJECTS.invite, emailHtml)
        ...
        if (postAction === 'resend_pending') {
          ...
          for (const member of pendingInvites) {
            try {
              ...
              // Send email with proxy link
              const emailHtml = generateInviteEmailHtml(inviteUrl, member.name)
              await sendEmailViaMicrosoftGraph(msToken, member.email, EMAIL_SUBJECTS.invite, emailHtml)
